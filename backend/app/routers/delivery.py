...
def create_output(payload: schemas.OutputCreate, db: Session = Depends(get_db)):
...
def update_output(output_id: int, payload: schemas.OutputUpdate, db: Session = Depends(get_db)):
...
def create_delivery_package(payload: schemas.DeliveryPackageCreate, db: Session = Depends(get_db)):
    episode = db.get(models.Episode, payload.episode_id)
    if not episode:
        raise HTTPException(status_code=404, detail="Episodio no encontrado")
...
def update_delivery_package(package_id: int, payload: schemas.DeliveryPackageUpdate, db: Session = Depends(get_db)):
