...
def test_episode_task_adr_flow(client):
    project = client.post(
    ...
    updated_task = client.patch(f"/api/tasks/{task['id']}", json={"status": "FINALIZADO"}).json()
    assert updated_task["status"] == "FINALIZADO"
    assert updated_task["resolved_at"] is not None

    adr_entry = client.post(
        "/api/adr",
        json={
    ...
    risk_run = client.post("/api/risks/run").json()
    assert risk_run["created"] >= 1

    risks = client.get("/api/risks", params={"project_id": project["id"]}).json()
